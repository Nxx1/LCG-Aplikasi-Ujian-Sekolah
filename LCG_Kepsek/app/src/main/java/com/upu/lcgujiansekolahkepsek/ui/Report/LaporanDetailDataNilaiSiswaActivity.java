...
import com.upu.lcgujiansekolahkepsek.adapter.Report.MyReportDetailDataNilaiSiswa;
...
public class LaporanDetailDataNilaiSiswaActivity extends AppCompatActivity {

    RecyclerView recycler_detail_data_ujian;
    private MyReportDetailDataNilaiSiswa adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_laporan_detail_data_nilai_siswa);

        TextView tvHeader = findViewById(R.id.textView6);
        TextView tvSubtitle = findViewById(R.id.textView7);

        tvHeader.setText(new StringBuilder("Laporan\nNilai ").append(Common.siswaModelSelected.getNamalengkap()));
        tvSubtitle.setText(new StringBuilder("Daftar nilai ujian ").append(Common.siswaModelSelected.getNamalengkap()));

        dbNilai = FirebaseDatabase.getInstance().getReference(Common.SISWA_REF)
                .child(Common.siswaModelSelected.getNamalengkap()).child(Common.NILAI_REF);

        storage = FirebaseStorage.getInstance();
        storageReference = storage.getReference();

        recycler_detail_data_ujian = findViewById(R.id.recycler_data_detail_nilai_siswa_list);

        progressDialog = new ProgressDialog(LaporanDetailDataNilaiSiswaActivity.this);
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(LaporanDetailDataNilaiSiswaActivity.this);
    ...
    protected void onStart() {
        ...
        dbNilai.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                adapter= new MyReportDetailDataNilaiSiswa(LaporanDetailDataNilaiSiswaActivity.this, skorModelArrayList);
                recycler_detail_data_ujian.setAdapter(adapter);
            }

            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(LaporanDetailDataNilaiSiswaActivity.this, "Terjadi kesalahan." +databaseError.getMessage().toString(), Toast.LENGTH_SHORT).show();
            }
        });
    }
}